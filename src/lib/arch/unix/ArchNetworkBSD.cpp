...
ArchNetworkBSD::~ArchNetworkBSD() { ARCH->closeMutex(m_mutex); }
...
void ArchNetworkBSD::throwErrorOnSocket(ArchSocket s) {
  ...
  if (getsockopt(s->m_fd, SOL_SOCKET, SO_ERROR, (optval_t*)&err, &size) == -1) {
    err = errno;
  }
...
bool ArchNetworkBSD::setNoDelayOnSocket(ArchSocket s, bool noDelay) {
  ...
  if (getsockopt(s->m_fd, IPPROTO_TCP, TCP_NODELAY, (optval_t*)&oflag, &size) ==
      -1) {
    throwError(errno);
  }
  ...
  if (setsockopt(s->m_fd, IPPROTO_TCP, TCP_NODELAY, (optval_t*)&flag, size) ==
      -1) {
    throwError(errno);
  }
...
bool ArchNetworkBSD::setReuseAddrOnSocket(ArchSocket s, bool reuse) {
  ...
  int oflag;
  socklen_t size = (socklen_t)sizeof(oflag);
  if (getsockopt(s->m_fd, SOL_SOCKET, SO_REUSEADDR, (optval_t*)&oflag, &size) ==
      -1) {
    throwError(errno);
  }
  ...
  int flag = reuse ? 1 : 0;
  size = (socklen_t)sizeof(flag);
  if (setsockopt(s->m_fd, SOL_SOCKET, SO_REUSEADDR, (optval_t*)&flag, size) ==
      -1) {
    throwError(errno);
  }
...
ArchNetAddress ArchNetworkBSD::nameToAddr(const std::string& name) {
  // allocate address
  ArchNetAddressImpl* addr = new ArchNetAddressImpl;
...
IArchNetwork::EAddressFamily ArchNetworkBSD::getAddrFamily(
    ArchNetAddress addr) {
  assert(addr != NULL);
...
bool ArchNetworkBSD::isAnyAddr(ArchNetAddress addr) {
  ...
  switch (getAddrFamily(addr)) {
    ...
    case kINET6: {
      auto* ipAddr = TYPED_ADDR(struct sockaddr_in6, addr);
      return (memcmp(&ipAddr->sin6_addr, &in6addr_any, sizeof(in6addr_any)) ==
                  0 &&
              addr->m_len == (socklen_t)sizeof(struct sockaddr_in6));
    }
