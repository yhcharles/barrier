...
enum { kCarbonLoopWaitTimeout = 10 };
...
OSXScreen::OSXScreen(IEventQueue* events, bool isPrimary, bool autoShowHideCursor)
  ...
  try {
    ...
    InstallApplicationEventHandler(switchEventHandler, 2, switchEventTypes, this,
                                   &m_switchEventHandlerRef);
    DisposeEventHandlerUPP(switchEventHandler);

    constructMouseButtonEventMap();

    // watch for requests to sleep
    m_events->adoptHandler(m_events->forOSXScreen().confirmSleep(), getEventTarget(),
    ...
    *m_pmThreadReady = false;
    ...
    m_pmWatchThread =
        new Thread(new TMethodJob<OSXScreen>(this, &OSXScreen::watchSystemPowerThread));
  } catch (...) {
...
void* OSXScreen::getEventTarget() const { return const_cast<OSXScreen*>(this); }
...
SInt32 OSXScreen::getJumpZoneSize() const { return 1; }
...
UInt32 OSXScreen::registerHotKey(KeyID key, KeyModifierMask mask) {
  ...
  if (!okay) {
    ...
    LOG((CLOG_WARN "failed to register hotkey %s (id=%04x mask=%04x)",
         barrier::KeyMap::formatKey(key, mask).c_str(), key, mask));
    return 0;
  }

  m_hotKeys.insert(std::make_pair(id, HotKeyItem(ref, macKey, macMask)));

  LOG((CLOG_DEBUG "registered hotkey %s (id=%04x mask=%04x) as id=%d",
       barrier::KeyMap::formatKey(key, mask).c_str(), key, mask, id));
  return id;
}
...
void OSXScreen::unregisterHotKey(UInt32 id) {
  ...
  if (i->second.getRef() != NULL) {
    ...
    for (ModifierHotKeyMap::iterator j = m_modifierHotKeys.begin(); j != m_modifierHotKeys.end();
         ++j) {
      if (j->second == id) {
  ...
  if (!press && (id == kButtonLeft)) {
    if (m_fakeDraggingStarted) {
      m_getDropTargetThread =
          new Thread(new TMethodJob<OSXScreen>(this, &OSXScreen::getDropTargetThread));
    }
...
void OSXScreen::fakeMouseWheel(SInt32 xDelta, SInt32 yDelta) const {
  if (xDelta != 0 || yDelta != 0) {
    ...
    CGEventRef scrollEvent = CGEventCreateScrollWheelEvent(NULL, kCGScrollEventUnitLine, 2,
...
void OSXScreen::showCursor() {
  ...
  CFStringRef propertyString =
      CFStringCreateWithCString(NULL, "SetsCursorInBackground", kCFStringEncodingMacRoman);

  CGSSetConnectionProperty(_CGSDefaultConnection(), _CGSDefaultConnection(), propertyString,
                           kCFBooleanTrue);
...
void OSXScreen::enable() {
  ...
  m_events->adoptHandler(Event::kTimer, m_clipboardTimer,
                         new TMethodEventJob<OSXScreen>(this, &OSXScreen::handleClipboardCheck));
  ...
  if (m_isPrimary) {
    ...
    m_eventTapPort =
        CGEventTapCreate(kCGHIDEventTap, kCGHeadInsertEventTap, kCGEventTapOptionDefault,
                         kCGEventMaskForAllEvents, handleCGInputEvent, this);
  } else {
    ...
    m_eventTapPort =
        CGEventTapCreate(kCGHIDEventTap, kCGHeadInsertEventTap, kCGEventTapOptionDefault,
                         kCGEventMaskForAllEvents, handleCGInputEventSecondary, this);
  }
...
void OSXScreen::setSequenceNumber(UInt32 seqNum) { m_sequenceNumber = seqNum; }

bool OSXScreen::isPrimary() const { return m_isPrimary; }
    ...
    case kEventClassMouse:
      switch (GetEventKind(*carbonEvent)) {
        case kBarrierEventMouseScroll: {
          ...
          // get scroll amount
          r = GetEventParameter(*carbonEvent, kBarrierMouseScrollAxisX, typeSInt32, NULL,
                                sizeof(xScroll), NULL, &xScroll);
          if (r != noErr) {
            xScroll = 0;
          }
          r = GetEventParameter(*carbonEvent, kBarrierMouseScrollAxisY, typeSInt32, NULL,
                                sizeof(yScroll), NULL, &yScroll);
          if (r != noErr) {
            yScroll = 0;
          }

          if (xScroll != 0 || yScroll != 0) {
            onMouseWheel(-mapScrollWheelToBarrier(xScroll), mapScrollWheelToBarrier(yScroll));
          }
        }
      }
      break;
...
void OSXScreen::handleClipboardCheck(const Event&, void*) { checkClipboards(); }

void OSXScreen::displayReconfigurationCallback(CGDirectDisplayID displayID,
  ...
  if (eventKind == kCGEventFlagsChanged) {
    ...
    if (m_activeModifierHotKey == 0) {
      if (m_modifierHotKeys.count(newMask) > 0) {
        ...
        m_events->addEvent(Event(m_events->forIPrimaryScreen().hotKeyDown(), getEventTarget(),
    ...
    // then generate a hot key up event.
      ...
      if (mask != m_activeModifierHotKeyMask) {
        m_events->addEvent(Event(m_events->forIPrimaryScreen().hotKeyUp(), getEventTarget(),
                                 HotKeyInfo::alloc(m_activeModifierHotKey)));
        m_activeModifierHotKey = 0;
        m_activeModifierHotKeyMask = 0;
      }
...
void OSXScreen::onMediaKey(CGEventRef event) {
  ...
  LOG((CLOG_DEBUG2 "Media key event: keyID=0x%02x, %s, repeat=%s", keyID, (down ? "down" : "up"),
       (isRepeat ? "yes" : "no")));
...
bool OSXScreen::onHotKey(EventRef event) const {
  // get the hotkey id
  EventHotKeyID hkid;
  GetEventParameter(event, kEventParamDirectObject, typeEventHotKeyID, NULL, sizeof(EventHotKeyID),
                    NULL, &hkid);
  UInt32 id = hkid.id;
...
ButtonID OSXScreen::mapBarrierButtonToMac(UInt16 button) const {
  switch (button) {
    case 1:
      return kButtonLeft;
    case 2:
...
ButtonID OSXScreen::mapMacButtonToBarrier(UInt16 macButton) const {
  switch (macButton) {
    case 1:
      return kButtonLeft;
...
SInt32 OSXScreen::mapScrollWheelToBarrier(float x) const {
...
SInt32 OSXScreen::mapScrollWheelFromBarrier(float x) const {
...
double OSXScreen::getScrollSpeed() const {
  ...
  CFPropertyListRef pref =
      ::CFPreferencesCopyValue(CFSTR("com.apple.scrollwheel.scaling"), kCFPreferencesAnyApplication,
...
double OSXScreen::getScrollSpeedFactor() const { return pow(10.0, getScrollSpeed()); }
...
void OSXScreen::enableDragTimer(bool enable) {
  if (enable && m_dragTimer == NULL) {
    ...
    m_events->adoptHandler(Event::kTimer, m_dragTimer,
                           new TMethodEventJob<OSXScreen>(this, &OSXScreen::handleDrag));
    CGEventRef event = CGEventCreate(NULL);
    CGPoint mouse = CGEventGetLocation(event);
...
IKeyState* OSXScreen::getKeyState() const { return m_keyState; }

void OSXScreen::updateScreenShape(const CGDirectDisplayID,
...
void OSXScreen::updateScreenShape() {
  ...
  LOG((CLOG_DEBUG "screen shape: center=%d,%d size=%dx%d on %u %s", m_x, m_y, m_w, m_h,
...
pascal OSStatus OSXScreen::userSwitchCallback(EventHandlerCallRef nextHandler, EventRef theEvent,
                                              void* inUserData) {
  OSXScreen* screen = (OSXScreen*)inUserData;
...
void OSXScreen::powerChangeCallback(void* refcon, io_service_t service, natural_t messageType,
                                    void* messageArg) {
...
void OSXScreen::handlePowerChangeRequest(natural_t messageType, void* messageArg) {
  ...
  switch (messageType) {
    case kIOMessageSystemWillSleep:
      ...
      m_events->addEvent(Event(m_events->forOSXScreen().confirmSleep(), getEventTarget(),
                               messageArg, Event::kDontFreeData));
      return;
...
void OSXScreen::handleConfirmSleep(const Event& event, void*) {
  ...
  if (messageArg != 0) {
    ...
    if (m_pmRootPort != 0) {
      ...
      m_events->addEvent(Event(m_events->forIScreen().suspend(), getEventTarget(), NULL,
...
OSXScreen::HotKeyItem::HotKeyItem(UInt32 keycode, UInt32 mask)
    : m_ref(NULL), m_keycode(keycode), m_mask(mask) {
  // do nothing
}

OSXScreen::HotKeyItem::HotKeyItem(EventHotKeyRef ref, UInt32 keycode, UInt32 mask)
    : m_ref(ref), m_keycode(keycode), m_mask(mask) {
  // do nothing
}

EventHotKeyRef OSXScreen::HotKeyItem::getRef() const { return m_ref; }
...
// off the screen.
CGEventRef OSXScreen::handleCGInputEventSecondary(CGEventTapProxy proxy, CGEventType type,
...
CGEventRef OSXScreen::handleCGInputEvent(CGEventTapProxy proxy, CGEventType type, CGEventRef event,
                                         void* refcon) {
  OSXScreen* screen = (OSXScreen*)refcon;
  CGPoint pos;
  ...
  switch (type) {
    ...
    case kCGEventOtherMouseDown:
      screen->onMouseButton(true,
                            CGEventGetIntegerValueField(event, kCGMouseEventButtonNumber) + 1);
      break;
    case kCGEventLeftMouseUp:
    case kCGEventRightMouseUp:
    case kCGEventOtherMouseUp:
      screen->onMouseButton(false,
                            CGEventGetIntegerValueField(event, kCGMouseEventButtonNumber) + 1);
      break;
...
bool OSXScreen::MouseButtonState::any() { return m_buttons.any(); }

void OSXScreen::MouseButtonState::reset() { m_buttons.reset(); }

void OSXScreen::MouseButtonState::overwrite(UInt32 buttons) {
  m_buttons = std::bitset<NumButtonIDs>(buttons);
}

bool OSXScreen::MouseButtonState::test(UInt32 button) const { return m_buttons.test(button); }
...
void setZeroSuppressionInterval() { CGSetLocalEventsSuppressionInterval(0.0); }
