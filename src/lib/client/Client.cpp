...
Client::Client(IEventQueue* events, const std::string& name,
...
void Client::connect() {
  ...
  try {
    ...
    IDataSocket* socket = m_socketFactory->create(
        ARCH->getAddrFamily(m_serverAddress.getAddress()), m_useSecureNetwork);
...
void Client::handleHello(const Event&, void*) {
  ...
  ProtocolUtil::writef(m_stream, kMsgHelloBack, kProtocolMajorVersion,
...
void Client::onFileRecieveCompleted() {
  if (isReceivedFileSizeValid()) {
    m_writeToDropDirThread =
        new Thread(new TMethodJob<Client>(this, &Client::writeToDropDirThread));
  }
}
...
void Client::sendFileToServer(const char* filename) {
  ...
  m_sendFileThread = new Thread(
      new TMethodJob<Client>(this, &Client::sendFileThread,
