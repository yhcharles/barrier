...
void ServerApp::parseArgs(int argc, const char* const* argv) {
  ...
  if (!result || args().m_shouldExit) {
    ...
    if (!args().m_barrierAddress.empty()) {
      try {
        *m_barrierAddress =
            NetworkAddress(args().m_barrierAddress, kDefaultPort);
        m_barrierAddress->resolve();
      } catch (XSocketAddress& e) {
        LOG((CLOG_PRINT "%s: %s" BYE, args().m_exename.c_str(), e.what(),
...
#define WINAPI_INFO ""
  ...
  buffer
      ...
      << " [--config <pathname>]" << WINAPI_ARGS << HELP_SYS_ARGS
      << HELP_COMMON_ARGS << "\n"
      << "\n"
      << "Options:\n"
      ...
      << "Placing brackets around an IPv6 address is required when also "
         "specifying \n"
      << "a port number and optional otherwise. The default is to listen on "
         "all\n"
...
void ServerApp::loadConfig() {
  ...
  if (!loaded) {
    LOG((CLOG_PRINT "%s: no configuration available",
         args().m_exename.c_str()));
    m_bye(kExitConfig);
  }
}
...
void ServerApp::closeServerScreen(barrier::Screen* screen) {
  if (screen != NULL) {
...
barrier::Screen* ServerApp::openServerScreen() {
...
static const char* const family_string(IArchNetwork::EAddressFamily family) {
  if (family == IArchNetwork::kINET) return "IPv4";
...
bool ServerApp::startServer() {
  ...
  try {
    ...
    auto family =
        family_string(ARCH->getAddrFamily(listenAddress.getAddress()));
...
barrier::Screen* ServerApp::createScreen() {
#if WINAPI_MSWINDOWS
  return new barrier::Screen(
      new MSWindowsScreen(true, args().m_noHooks, args().m_stopOnDeskSwitch,
                          m_events),
      m_events);
#elif WINAPI_XWINDOWS
  return new barrier::Screen(
      new XWindowsScreen(new XWindowsImpl(), args().m_display, true,
...
ClientListener* ServerApp::openClientListener(const NetworkAddress& address) {
  ClientListener* listen = new ClientListener(
      address, new TCPSocketFactory(m_events, getSocketMultiplexer()), m_events,
      args().m_enableCrypto);
    ...
    if (!access(args().m_screenChangeScript.c_str(), X_OK)) {
      ...
      if (pid == 0) {
        execl(args().m_screenChangeScript.c_str(),
              args().m_screenChangeScript.c_str(), info->m_screen, NULL);
        exit(0);
      } else if (pid < 0) {
      ...
      LOG((CLOG_ERR "Script not accessible \"%s\"",
